<x-guest-layout>
    ...
    <section class="w-full h-screen flex flex-col justify-center items-center bg-gray-100">
        <div class="w-[100%] max-w-4xl flex justify-start items-center">
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
            ...
            if (checker.classList.contains('king')) {
                ...
                if (distance === 1 && Math.abs(startCol - endCol) === 1) {
                    ...
                    const middleCell = document.querySelector(
                        `td[data-row="${middleRow}"][data-col="${middleCol}"]`);
                    if (middleCell && middleCell.firstChild && middleCell.firstChild.dataset.color !== checker
                        .dataset.color) {
                        return true;
                    }
                }
        ...
        function isPathClear(startRow, startCol, endRow, endCol) {
            ...
            let pathClear = true;
            for (let row = startRow + rowStep, col = startCol + colStep; row !== endRow && col !==
                endCol; row += rowStep, col += colStep) {
        ...
        function moveChecker(checker, destination) {
            ...
            if ((row === 0 && checker.dataset.color === playerColor) || (row === boardSize - 1 && checker
                    .dataset.color === computerColor)) {
        ...
        function canCaptureAgain(checker, row, col) {
            ...
            return directions.some(({
                ...
                if (newRow >= 0 && newRow < boardSize && newCol >= 0 && newCol < boardSize) {
                    ...
                    return middleCell && middleCell.firstChild && middleCell.firstChild.dataset
                        .color !== checker.dataset.color && targetCell && !targetCell.firstChild;
                }
                return false;
            });
        }
        ...
        function computerMove() {
            ...
            const computerCheckers = allCells.filter(cell => cell.firstChild && cell.firstChild.dataset
                .color === computerColor);
            ...
            computerCheckers.forEach(checkerCell => {
                ...
                moves.forEach(move => possibleMoves.push({
                    startRow,
                    startCol,
                    move
                }));
            ...
            if (movesToConsider.length > 0) {
                ...
                const startCell = document.querySelector(
                    `td[data-row="${randomMove.startRow}"][data-col="${randomMove.startCol}"]`);
                const targetCell = document.querySelector(
                    `td[data-row="${randomMove.move.row}"][data-col="${randomMove.move.col}"]`);
                const capturedPiece = capturePieceIfNeeded(randomMove.startRow, randomMove.startCol, randomMove
                    .move.row, randomMove.move.col);
                moveChecker(startCell.firstChild, targetCell);

                // Movimiento de captura adicional si es posible
                const newMoves = getPossibleMoves(randomMove.move.row, randomMove.move.col, targetCell
                    .firstChild);
                if (capturedPiece && newMoves.length > 0) {
                    const nextMove = newMoves[Math.floor(Math.random() * newMoves.length)];
                    const nextTargetCell = document.querySelector(
                        `td[data-row="${nextMove.row}"][data-col="${nextMove.col}"]`);
        ...
        function getPossibleMoves(row, col, checker) {
            ...
            for (const {
                ...
                while (newRow >= 0 && newRow < boardSize && newCol >= 0 && newCol < boardSize) {
                    ...
                    } else if (targetCell && targetCell.firstChild && targetCell.firstChild.dataset.color !==
                        checker.dataset.color) {
                        const captureRow = newRow + rowDir;
                        const captureCol = newCol + colDir;
                        const captureCell = document.querySelector(
                            `td[data-row="${captureRow}"][data-col="${captureCol}"]`);
        ...
        function checkWin(color) {
            const allCells = Array.from(document.querySelectorAll('.black-cell'));
            const playerCheckers = allCells.filter(cell => cell.firstChild && cell.firstChild.dataset.color ===
                playerColor);
            const computerCheckers = allCells.filter(cell => cell.firstChild && cell.firstChild.dataset
                .color === computerColor);
